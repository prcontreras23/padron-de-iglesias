...
export default async function handler(req, res) {
  ...
  if (action === 'crear') {
    ...
    const { ok, data } = await adminFetch('users', 'POST', { email, password, email_confirm: true, user_metadata: { nombre: nombre || '' } });
    if (!ok) return res.status(400).json({ error: data?.msg || data?.message || JSON.stringify(data) });
    const userId = data.id;
    await sbReq('profiles', 'POST', { id: userId, email, nombre: nombre || '', permisos: permisos || {}, activo: true, debe_cambiar_password: true });
    await sendEmail({
      to: email,
      subject: 'Tu acceso al Sistema de Iglesias — Asociación Dominicana del Sureste',
      html: emailBase({
        titulo: 'Bienvenido al Sistema de Gestión',
        cuerpo: `<p style="color:#374151;margin:0 0 12px;">Hola${nombre ? ' <strong>' + nombre + '</strong>' : ''},</p>
        <p style="color:#374151;margin:0 0 16px;">Se ha creado tu cuenta en el Sistema de Gestión de Iglesias.</p>
        <div style="background:#f3f4f6;border-radius:8px;padding:16px 20px;margin:0 0 16px;">
          <p style="margin:0 0 6px;color:#6b7280;font-size:13px;">Tus credenciales:</p>
        ...
        <p style="color:#374151;margin:0 0 4px;">Al iniciar sesión se te pedirá cambiar tu contraseña.</p>
        <p style="color:#ef4444;font-size:13px;margin:0;">No compartas estas credenciales.</p>`,
        boton: { url: SITE, texto: 'Ir al Sistema' },
      }),
    });
    return res.status(200).json({ ok: true, uid: userId, password });
  }
